    ...
    getFileInfo: function () {
        ...
        var files = Object.keys(Ext.ClassManager.classes)
            ...
            .map((className, index) => {
                ...
                //  */
                // (Ext.Loader.requiresMap[className] || []).forEach(cls => {
                //     requires.push(Ext.Loader.getPath(cls));
                // });
                var requires = Object.keys(Demo.view[package]).map(cls => {
                    return Ext.Loader.getPath(`Demo.view.${package}.${cls}`);
                });
                /**
                 * Since the requires are used to determine the source 
                 * code files but requires only wants js files.
